...
function getApproximateBlockchainHeight(_date, _nettype){
    ...
    var aeonBirthTime = _nettype == "Mainnet" ? 1402077642 : _nettype == "Testnet" ? 1410295020 : 1518932025;
    // avg seconds per block in v1
    var secondsPerBlockV1 = 60;
    // time of v2 fork 2016-03-23 15:57:38 (1458748658)
    var forkTime = _nettype == "Mainnet" ? 1438699524 : _nettype == "Testnet" ? 1448285909 : 1520937818;
    // v2 fork block
    var forkBlock = _nettype == "Mainnet" ? 592000 : _nettype == "Testnet" ? 624634 : 32000;
    // avg seconds per block in V2
    var secondsPerBlockV2 = 240;
    // time in UTC
    var requestedTime = Math.floor(new Date(_date) / 1000);
    var approxBlockchainHeight;
    var secondsPerBlock;
    // before monero's birth
    if (requestedTime < aeonBirthTime){
        console.log("Calculated blockchain height: 0, requestedTime < aeonBirthTime " );
        return 0;
    }
    // time between during v1
    if (requestedTime > aeonBirthTime && requestedTime < forkTime){
        approxBlockchainHeight = Math.floor((requestedTime - aeonBirthTime)/secondsPerBlockV1);
